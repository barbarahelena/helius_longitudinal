#!/usr/bin/env python3
"""
Create simple samplesheet for bins from bin_summary.tsv.
Generates a CSV with: bin_id, bin_fasta
...
def create_bin_samplesheet(input_files, output_file, bins_dirs=None):
    """
    Create simple samplesheet from bin_summary.tsv
    ...
    for i, input_file in enumerate(input_files):
        ...
        # Column names: bin, Name
        required_cols = ['bin', 'Name']
        ...
        # Keep only bin_id and bin_fasta columns
        df_output = df_output[['bin_id', 'bin_fasta']]
    ...
    print(f"\nCreated samplesheet with {len(df_combined)} bins")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python create_bin_samplesheet.py <input_file1> [input_file2 ...] <output_file> [bins_dir1 bins_dir2 ...]")
        print("\nArguments:")
        ...
        print("\nExamples:")
        print("  Single file:")
        print("    python create_bin_samplesheet.py bin_summary.tsv bin_samplesheet.csv")
        print("\n  Multiple batches (auto-detect bins directories):")
        print("    python create_bin_samplesheet.py batch1/bin_summary.tsv batch2/bin_summary.tsv batch3/bin_summary.tsv bin_samplesheet.csv")
        print("\n  Multiple batches (specify bins directories):")
        print("    python create_bin_samplesheet.py batch1/bin_summary.tsv batch2/bin_summary.tsv bin_samplesheet.csv batch1/bins batch2/bins")
        sys.exit(1)
    ...
    create_bin_samplesheet(input_files, output_file, bins_dirs)
