...
df_new2 <- df_new %>% 
    mutate(across(where(is.character), ~na_if(., c("Missing", "Missing: n.v.t.", "niet ingevuld","nvt", 
           ...
           across(c("GlucLowDrugs_BA", "GlucLowDrugs_FU"), ~fct_recode(factor(.x), "Yes" = "Ja", "No" = "Nee")),
           across(c("Cortico_BA", "Cortico_FU"), ~fct_recode(factor(.x), "Yes" = "Ja", "No" = "Nee")),
           Smoking_current_BA = fct_collapse(Smoking_BA, "Yes" = "Yes", "No" = c("Never", "Former smoking")),
           Smoking_current_FU = fct_collapse(Smoking_FU, "Yes" = "Yes", "No" = c("Never", "Former smoking")),
         ...
         Dyslipidemia_BA = as.factor(case_when(LLD_BA == "Yes" | LDL_BA > 3.0 ~ "Yes", .default = "No")),
         Dyslipidemia_FU = as.factor(case_when(LLD_FU == "Yes" | LDL_FU > 3.0 ~ "Yes", .default = "No")),
...
fu <- data.frame(ID = idsfu, h2_16s = TRUE)
idsba <- str_remove(sample_names(heliusmb)[which(str_detect(sample_names(heliusmb), "HELIBA_"))], "HELIBA_")
ba <- data.frame(ID = idsba, h1_16s = TRUE)
totids <- full_join(fu, ba, by = "ID")
totids <- totids |> mutate(h2_16s = case_when(is.na(h2_16s) ~ FALSE, .default = h2_16s),
                            h1_16s = case_when(is.na(h1_16s) ~ FALSE, .default = h1_16s))
write.csv(totids, "data/16s/ids16s_april2026.csv")
