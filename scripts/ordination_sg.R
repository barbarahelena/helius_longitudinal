...
ev_bray <- read.csv("results/ordination/expl_var_bray_shotgun.csv", header = FALSE)
...
ggsave(braycurt, filename = "results/ordination/PCoA_BrayCurtis_sg.pdf", device = "pdf", width = 8, height = 8)
...
ggplot(data = heliusdist %>% filter(!is.na(EthnicityTot)), 
    ...
    theme_Publication() +
    coord_flip()
ggsave("results/ordination/sg_distance_ethnicities.pdf", width = 6, height = 4)
...
ggplot(data = heliusdist %>% filter(!is.na(FUtime)), 
    ...
    theme_Publication() +
    coord_flip()
ggsave("results/ordination/sg_futime_ethnicities.pdf", width = 6, height = 4)
...
ggplot(data = heliusdist %>% filter(!is.na(HT_BPMed)), aes(x = HT_BPMed, y = distance)) +
    ...
    stat_compare_means(tip.length = 0, hide.ns = TRUE,
                       label = "p.signif") +
    theme_Publication()
ggsave("results/ordination/sg_distance_hypertension.pdf", width = 4, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(HT_BPMed)), aes(x = HT_BPMed, y = distance)) +
    ...
    theme_Publication()
ggsave("results/ordination/sg_distance_hypertension_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(DM)), aes(x = DM, y = distance)) +
    ...
    stat_compare_means(tip.length = 0, hide.ns = TRUE,
                       label = "p.signif") +
    theme_Publication()
ggsave("results/ordination/sg_distance_dm.pdf", width = 4, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(HT_BPMed)), aes(x = HT_BPMed, y = distance)) +
    ...
    facet_wrap(~Ethnicity) +
    theme_Publication()
ggsave("results/ordination/sg_distance_diabetes_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(MetSyn)), aes(x = MetSyn, y = distance)) +
    ...
    facet_wrap(~Ethnicity) +
    theme_Publication()
ggsave("results/ordination/distance_metsyn_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(HbA1c_delta)), aes(x = distance, y = HbA1c_delta)) +
    ...
    stat_cor() +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltahba1c.pdf", width = 4.5, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(HbA1c_delta)), aes(x = distance, y = HbA1c_delta)) +
    ...
    stat_cor() +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltahba1c_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(LDL_delta)), aes(x = distance, y = LDL_delta)) +
    ...
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltaldl.pdf", width = 4.5, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(LDL_delta)), aes(x = distance, y = LDL_delta)) +
    ...
    stat_cor(method = "spearman") +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltaldl_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(Age)), aes(x = Age, y = distance)) +
    ...
    stat_cor(method = "spearman") +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_baselineage.pdf", width = 4.5, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(Age)), aes(x = Age, y = distance)) +
    ...
    stat_cor(method = "spearman") +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_baseage_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(BMI_delta)), aes(x = BMI_delta, y = distance)) +
    ...
    stat_cor() +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltabmi.pdf", width = 4.5, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(BMI_delta)), aes(x = distance, y = BMI_delta)) +
    ...
    stat_cor() +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_deltabmi_ethnicity.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(FUtime)), aes(x = FUtime, y = distance)) +
    ...
    stat_cor() +
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_futime.pdf", width = 4.5, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(FUtime)), aes(x = FUtime, y = distance)) +
    ...
    theme_Publication()
ggsave("results/ordination/sg_braycurtis_futime_ethnicity.pdf", width = 6, height = 5)

# Differences between baseline diagnoses and controls stratified for EthnicityTot
...
ggplot(data = heliusdist %>% filter(!is.na(HT_BPMed)), aes(x = HT_BPMed, y = distance)) +
    ...
    facet_wrap(~EthnicityTot) +
    theme_Publication()
ggsave("results/ordination/sg_distance_ethnicity_hypertension.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(DM)), aes(x = DM, y = distance)) +
    ...
    facet_wrap(~EthnicityTot) +
    theme_Publication()
ggsave("results/ordination/sg_distance_ethnictiy_diabetes.pdf", width = 6, height = 5)
...
ggplot(data = heliusdist %>% filter(!is.na(MetSyn)), aes(x = MetSyn, y = distance)) +
    ...
    theme_Publication()
ggsave("results/ordination/sg_distance_ethnicity_metsyn.pdf", width = 6, height = 5)
...
(pl1 <- heliusdist %>% filter(!is.na(DM_new)) %>%  
     ...
     stat_compare_means(comparisons = list(c("Yes", "No")), tip.length = 0, hide.ns = TRUE,
                        label = "p.format") +
     theme_Publication())
ggsave("results/ordination/sg_newdiabetes.pdf", width = 4.5, height = 5)
...
(pl1 <- heliusdist %>% filter(!is.na(HT_new)) %>%  
        ...
        stat_compare_means(comparisons = list(c("Yes", "No")), tip.length = 0, hide.ns = TRUE,
                           label = "p.format") +
        theme_Publication())
ggsave("results/ordination/sg_newhypertension.pdf", width = 4.5, height = 5)
...
(pl1 <- heliusdist %>% filter(!is.na(MetSyn_new)) %>%  
        ...
        stat_compare_means(comparisons = list(c("Yes", "No")), tip.length = 0, hide.ns = TRUE,
                           label = "p.format") +
        theme_Publication())
ggsave("results/ordination/sg_newmetsyn.pdf", width = 4.5, height = 5)
...
(pl1 <- heliusdist %>% filter(!is.na(DM_new)) %>%  
     ...
     facet_wrap(~EthnicityTot) +
     theme_Publication())
ggsave("results/ordination/sg_newdiabetes_ethnicity.pdf", width = 6, height = 5)
...
(pl1 <- heliusdist %>% filter(!is.na(HT_new)) %>%  
        ...
        facet_wrap(~EthnicityTot) +
        theme_Publication())
ggsave("results/ordination/sg_newhypertension_ethnicity.pdf", width = 6, height = 5)
