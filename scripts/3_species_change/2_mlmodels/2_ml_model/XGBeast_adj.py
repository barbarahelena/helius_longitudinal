...
def main(args):
    ...
    for samples, test in ShufSpl.split(X, y):
        ...
        if tune_parameters:
            ...
            print("Optimizing parameters based on the grid:\n")
            print(param_grid)
    ...
    if model_type == "XGB_class":
        ...
        ax.plot(
            [0, 1], [0, 1], linestyle="--", lw=1.2, color="#999999", label="Random"
        )
        ax.set_xlim([0.0, 1.0])
        ...
        fig.tight_layout()
        fig.savefig(output_folder + "/Plot_AUC.pdf", bbox_inches="tight")
        plt.close(fig)
    ...
    # save environment package list to file
    import subprocess
    with open(output_folder + "/conda_environment.txt", "w") as f:
