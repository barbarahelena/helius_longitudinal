...
plot_sig <- function(tab, clindf, mbdf){
    ...
    for(a in colnames(tot)[2:ncol(tot)]){
        ...
        pl <- ggplot(tot, aes(x = var, y = dep+1)) +
                    scale_y_log10() +
                    geom_violin(colour = NA, aes(fill = var)) +
...
write.csv2(tab, file = "results/3_species_change/5_mlmodels/wilcoxon_timepoint_16s.csv")
...
head(tabdutch)
write.csv2(tabdutch, file = "results/3_species_change/5_mlmodels/wilcoxon_dutchtime_16s.csv")
...
head(tabsas)
write.csv2(tabsas, file = "results/3_species_change/5_mlmodels/wilcoxon_sastime_16s.csv")
...
head(tabba)
write.csv2(tabba, file = "results/3_species_change/5_mlmodels/wilcoxon_ethbase_16s.csv")
...
head(tabfu)
write.csv2(tabfu, file = "results/3_species_change/5_mlmodels/wilcoxon_ethfu_16s.csv")
