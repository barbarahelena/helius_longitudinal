...
dftot %>% group_by(EthnicityTot) %>% summarise(mean_sh = mean(sharing_perc, na.rm = TRUE), n_sh = length(sharing_perc), .groups = "drop_last")
...
(pl_fig3_J <- ggplot(data = dftot, aes(x = EthnicityTot, y = sharing_perc)) +
    ...
    theme_Publication())
ggsave("results/3_species_change/3_strain_stability/ethnicities.pdf", width = 4.5, height = 5)
...
(pl_fig3_H <- ggplot(data = dftot %>% filter(!is.na(FUtime)), aes(x = FUtime, y = sharing_perc)) +
    ...
    theme_Publication())
ggsave("results/3_species_change/3_strain_stability/futime.pdf", width = 4.5, height = 5)
...
(pl_fig3_I <- ggplot(data = dftot %>% filter(!is.na(FUtime)), aes(x = distance, y = sharing_perc)) +
    ...
    stat_cor_fmt(label.y = 100) +
    theme_Publication())
ggsave("results/3_species_change/3_strain_stability/bray_strainsharing.pdf", width = 4.5, height = 5)
...
(pl_fig3_G <- ggplot(data = dftot %>% filter(!is.na(shannon)), aes(x = shannon, y = sharing_perc)) +
    ...
    stat_cor_fmt(label.y = 100) +
    theme_Publication())
ggsave("results/3_species_change/3_strain_stability/shannon_strainsharing.pdf", width = 4.5, height = 5)
...
# Keep only SGBs with >50% prevalence in both ethnicities
...
(pl <- ggplot(data = taxtot %>% arrange(sharing_perc), 
...
(pl <- ggplot(data = difftrue, aes(x = Species)) +
    ...
    theme_Publication() + 
    coord_flip())
ggsave("results/3_species_change/3_strain_stability/diffsgbs_ethnicity_connect.pdf", width = 7, height = 7)
...
ggsave("results/3_species_change/3_strain_stability/diffsgbs_eth_dens.pdf", width = 10, height = 7)
...
## Figure panel: combined strain sharing plot (pl_fig3_K)
pl_fig3_K <- pl + dens + pl4 +
    plot_layout(widths = c(3, 1.2, 1.2), guides = "collect") &
    theme(legend.position = "bottom")
ggsave("results/3_species_change/3_strain_stability/diffstrains_complete.pdf", width = 15, height = 10)
