...
# ---- Figure 3: Delta heatmap (boss's request — for discussion) ----
# Requires subjects with both timepoints

paired_subj <- intersect(
    str_remove(sdf$sampleID[sdf$timepoint == "baseline"],  "HELIBA_"),
    str_remove(sdf$sampleID[sdf$timepoint == "follow-up"], "HELIFU_")
)
cat("Paired subjects:", length(paired_subj), "\n")

otu_base_paired <- otu_df |>
    filter(timepoint == "baseline") |>
    mutate(subject = str_remove(sampleID, "HELIBA_")) |>
    filter(subject %in% paired_subj) |>
    dplyr::select(subject, EthnicityTot, all_of(core_asvs))

otu_fu_paired <- otu_df |>
    filter(timepoint == "follow-up") |>
    mutate(subject = str_remove(sampleID, "HELIFU_")) |>
    filter(subject %in% paired_subj) |>
    dplyr::select(subject, all_of(core_asvs))

# Merge and compute delta (follow-up - baseline) per subject × ASV
otu_merged <- inner_join(otu_base_paired, otu_fu_paired,
                         by = "subject", suffix = c("_base", "_fu"))

otu_delta <- otu_base_paired |> dplyr::select(subject, EthnicityTot)
for (asv in core_asvs) {
    otu_delta[[asv]] <- otu_merged[[paste0(asv, "_fu")]] - otu_merged[[paste0(asv, "_base")]]
}

# Median delta per ethnic group
delta_mat <- matrix(NA_real_, nrow = length(core_asvs), ncol = length(all_groups),
                    dimnames = list(core_asvs, all_abbr))

for (j in seq_along(all_groups)) {
    sub <- if (all_groups[j] == "All") {
        otu_delta |> filter(EthnicityTot %in% eth_levels)
    } else {
        otu_delta |> filter(EthnicityTot == all_groups[j])
    }
    delta_mat[core_asvs, j] <- sapply(core_asvs, function(a) median(sub[[a]], na.rm = TRUE))
}

# Diverging color scale for delta
max_abs <- max(abs(delta_mat), na.rm = TRUE)
col_fun_delta <- colorRamp2(
    c(-max_abs, 0, max_abs),
...
ht_delta <- Heatmap(
    ...
    row_names_gp      = gpar(fontsize = 9),
    column_names_gp   = gpar(fontsize = 11, fontface = "bold"),
    row_labels        = row_labels_vec[rownames(delta_mat)],
    row_names_rot     = 0,
    column_names_rot  = 0,
    column_title      = "Delta (follow-up \u2212 baseline)",
    column_title_gp   = gpar(fontsize = 12, fontface = "bold"),
    show_heatmap_legend = TRUE,
    width             = unit(7, "cm")
)

CairoPDF(file.path(resultsfolder, "heatmap_16s_core_delta.pdf"), width = 9, height = 11)
draw(ht_delta, padding = unit(c(5, 60, 5, 5), "mm"))
dev.off()
